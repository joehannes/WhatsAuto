(ns whatsauto.views.sidebar
  "Navigation sidebar using ShadCN sidebar primitives.
   Shows all app navigation items with icons and unread badges."
  (:require
   [re-frame.core :as rf]
...
(def ^:private nav-items
  [{:id :chats    :label "Chats"        :icon ic/icon-chats}
   ...
   {:separator true}
   {:id :campaigns  :label "Campaigns"   :icon ic/icon-campaigns}
   ...
   {:separator true}
   {:id :knowledge  :label "Knowledge"   :icon ic/icon-knowledge}
   {:id :rules      :label "Rules"       :icon ic/icon-rules}
...
(defn- sidebar-logo []
  [:div.flex.items-center.gap-3.px-4.py-3
   ;; Teal circle with WA-style icon
